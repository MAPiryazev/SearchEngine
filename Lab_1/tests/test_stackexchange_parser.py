#!/usr/bin/env python3
"""
Unit-тесты для парсера Stack Exchange
"""
...
class TestStackExchangeParser:
    ...
    def test_clean_html_removes_code_blocks(self):
        """Проверка замены блоков кода"""
        parser = StackExchangeParser(None, None)
        
        text = "Текст <code>print('hello')</code> продолжение <pre>def foo():\n    pass</pre> конец"
        result = parser.clean_html(text)
    ...
    def test_clean_html_decodes_entities(self):
        """Проверка декодирования HTML entities"""
        parser = StackExchangeParser(None, None)
...
<posts>
  <row Id="1" PostTypeId="1" 
    ...
    def test_filters_answers(self, tmp_path):
        """Проверка фильтрации ответов (берём только вопросы)"""
        xml_content = '''<?xml version="1.0" encoding="utf-8"?>
<posts>
...
<posts>
  <row Id="3" PostTypeId="1" 
       ...
       CreationDate="2024-01-01T00:00:00.000" />
        ...
        # Короткий пост не должен попасть в результат (< 50 символов)
        assert output_file.stat().st_size == 0 or not output_file.exists()
