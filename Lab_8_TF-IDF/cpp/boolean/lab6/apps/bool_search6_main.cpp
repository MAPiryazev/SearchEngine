...
static int run_one_query(InvertedIndex6& inv,
                         ForwardIndex& fwd,
                         const std::string& q,
                         const TokenizeOptions& opt,
                         std::int64_t top_n) {
...
int main(int argc, char** argv) {
    try {
        std::string inv_path = get_arg(argc, argv, "--inv", "out6/index6.inv");
        std::string fwd_path = get_arg(argc, argv, "--fwd", "out6/index6.fwd");
        std::string q = get_arg(argc, argv, "--q", "");
        std::string qfile = get_arg(argc, argv, "--qfile", "");

        if (q.empty() && qfile.empty()) {
            std::cerr
                << "Usage: ./bool_search6 --q \"term1 AND term2\" [--inv out6/index6.inv --fwd out6/index6.fwd --top 20]\n"
                << "   or: ./bool_search6 --qfile queries.txt [--inv out6/index6.inv --fwd out6/index6.fwd --top 20]\n";
            return 2;
        }
        ...
        if (!qfile.empty()) {
            ...
            while (std::getline(fin, line)) {
                ...
                run_one_query(inv, fwd, line, opt, top_n);
            }
            return 0;
        }

        return run_one_query(inv, fwd, q, opt, top_n);
