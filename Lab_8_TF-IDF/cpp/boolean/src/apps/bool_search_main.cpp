...
static int run_one_query(InvertedIndex& inv,
                         ForwardIndex& fwd,
                         const std::string& q,
                         const TokenizeOptions& opt,
                         std::int64_t top_n) {
...
int main(int argc, char** argv) {
    try {
        ...
        if (q.empty() && qfile.empty()) {
            std::cerr
                << "Usage: ./bool_search --q \"term1 AND term2\" [--inv out/index.inv --fwd out/index.fwd --top 20]\n"
                << "   or: ./bool_search --qfile queries.txt [--inv out/index.inv --fwd out/index.fwd --top 20]\n";
            return 2;
        }
