...
int main(int argc, char** argv) {
    try {
        ...
        const char* out_c = get_arg(argc, argv, "--out", "out6");
        ...
        std::string fwd_path = std::string(out_c) + "/index6.fwd";
        {
        ...
        nostl::Vec<std::uint8_t> postings;
        ...
        while (i < occ.size) {
            ...
            while (i < occ.size && occ.data[i].term == term) {
                ...
                if (!lab6::vbyte_put_u32(postings, doc_gap)) throw std::runtime_error("oom");

                const std::uint32_t tf = static_cast<std::uint32_t>(j - i);
                if (!lab6::vbyte_put_u32(postings, tf)) throw std::runtime_error("oom");
                ...
                for (std::size_t k = i; k < j; ++k) {
                    ...
                    prev_pos = pos;
                    if (!lab6::vbyte_put_u32(postings, pos_gap)) throw std::runtime_error("oom");
                }
        ...
        std::string inv_path = std::string(out_c) + "/index6.inv";
        {
            ...
            if (postings.size) write_bytes(out, postings.data, postings.size);
        }
